...
export function renderAnnotation(a: Annotation, showLocation: boolean): string {
  ...
  const loc = showLocation && a.target?.filePath ? targetLabel(a.target) : "";
  const title =
    a.kind === "assessment"
  ...
  return `<div class="${annoContainerClass(a.kind)}" data-anno="${escapeHtml(a.id)}">
    <div class="${CLASS.annoHead}">
      <span class="${annoKindClass(a.kind)}">${a.kind}</span>
      <span class="${CLASS.annoTrack}">${escapeHtml(a.track)}</span>
      <span class="${CLASS.annoTitle}">${linkify(title)}</span>
      ${a.status ? `<span class="${factStatusClass(escapeHtml(a.status))}">${escapeHtml(a.status)}</span>` : ""}
      ${loc ? `<span class="${CLASS.annoLoc}">${loc}</span>` : ""}
      ${tags}
    </div>
    ${body}
    </div>`;
...
function annotationTags(a: Annotation): string[] {
  if (a.kind === "input-request") {
    return [a.mode, a.reasonCode].filter((value): value is string => !!value);
  }
  return a.kind === "observation" || a.kind === "validation"
    ? (a.tags ?? [])
    : [];
}

function targetLabel(target: AnnotationTarget | undefined): string {
  const t = target ?? {};
  if (t.kind === "range") {
    return `${escapeHtml(t.filePath)}:${t.startLine}-${t.endLine ?? t.startLine} (${escapeHtml(t.side || "new")})`;
  }
  if (t.kind === "file") return escapeHtml(t.filePath || "");
  if (t.kind === "revision") return "whole revision";
